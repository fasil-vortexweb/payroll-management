...
<body>
                    ...
                    <tbody>
                        ...
                        if (!empty($additions)) {
                            foreach ($additions as $addition) {
                                echo "<tr>
                                    <td>{$addition['ufCrm42_1726317240940']}</td>
                                    <td>{$addition['ufCrm42_1726318090948']}</td>
                                    <td>{$addition['ufCrm42_1726317366023']}</td>
                                    <td class='text-right'>
                    ...
                    <tbody>
                        ...
                        if (!empty($overtimes)) {
                            foreach ($overtimes as $overtime) {
                                echo "<tr>
                                    <td>{$overtime['ufCrm44_1726318399141']}</td>
                                    <td>{$overtime['ufCrm44_1726318406212']}</td>
                                    <td>{$overtime['ufCrm44_1726318414156']}</td>
                                    <td class='text-right'>
                    ...
                    <tbody>
                        ...
                        if (!empty($deductions)) {
                            foreach ($deductions as $deduction) {
                                echo "<tr>
                                    <td>{$deduction['ufCrm46_1726318456537']}</td>
                                    <td>{$deduction['ufCrm46_1726318467816']}</td>
                                    <td class='text-right'>
    ...
    <script>
        const PAYROLL_ADDITIONS_ENTITY_TYPE_ID = 1118;
        const PAYROLL_OVERTIMES_ENTITY_TYPE_ID = 1122;
        const PAYROLL_DEDUCTIONS_ENTITY_TYPE_ID = 1126;
        const WEBHOOK_BASE_URL = 'https://b24-g51s9r.bitrix24.com/rest/8/6596tfx7g78yi7p1/'
        ...
        $(document).ready(function() {
            ...
            $('.addition-edit-btn').click(async function() {
                ...
                await fetch(`${WEBHOOK_BASE_URL}crm.item.get.json?entityTypeId=${PAYROLL_ADDITIONS_ENTITY_TYPE_ID}&id=${Number(editAdditionId)}`)
                    ...
                    .then(item => {
                        document.getElementById('editAdditionName').value = item.ufCrm42_1726317240940;
                        document.getElementById('editAdditionCategory').value = item.ufCrm42_1726318090948;
                        document.getElementById('editAdditionUnitAmount').value = item.ufCrm42_1726317366023;
                    })
                    .catch(error => {
            ...
            $('.overtime-edit-btn').click(async function() {
                ...
                await fetch(`${WEBHOOK_BASE_URL}crm.item.get.json?entityTypeId=${PAYROLL_OVERTIMES_ENTITY_TYPE_ID}&id=${Number(editOvertimeId)}`)
                    ...
                    .then(data => data.result.item)
                    .then(item => {
                        document.getElementById('editOvertimeName').value = item.ufCrm44_1726318399141;
                        document.getElementById('editOvertimeCategory').value = item.ufCrm44_1726318406212;
                        document.getElementById('editOvertimeRate').value = item.ufCrm44_1726318414156;
                    })
                    .catch(error => {
                        console.error('Error:', error);
                    })
            ...
            $('.deduction-edit-btn').click(async function() {
                ...
                await fetch(`${WEBHOOK_BASE_URL}crm.item.get.json?entityTypeId=${PAYROLL_DEDUCTIONS_ENTITY_TYPE_ID}&id=${Number(editDeductionId)}`)
                    ...
                    .then(data => data.result.item)
                    .then(item => {
                        document.getElementById('editDeductionName').value = item.ufCrm46_1726318456537;
                        document.getElementById('editDeductionUnitAmount').value = item.ufCrm46_1726318467816;
                    })
                    .catch(error => {
                        console.error('Error:', error);
                    })
